...
class Menu extends Component
{
    ...
    /**
     * Routes a platform administrator may reach without a working school.
     */
    ...
    public function mount()
    {
        ...
        $this->menu = $this->limitToPlatformMenu($this->menu);
    }

    /**
     * A platform administrator without a working school sees platform
     * navigation only. School data (fees, parents, teachers, terms…)
     * stays hidden until a school is picked.
     */
    private function limitToPlatformMenu(array $menu): array
    {
        $user = auth()->user();

        if ($user === null || !$user->isPlatformAdmin() || school_context()->has()) {
            return $menu;
        }
        ...
        foreach ($menu as $item) {
            if (isset($item['header'])) {
                if (in_array($item['header'], self::PLATFORM_HEADERS, true)) {
                    $filtered[] = $item;
                }
            ...
            foreach ($item['submenu'] ?? [] as $sub) {
                $routes[] = $sub['route'] ?? null;
            }

            if (array_intersect($routes, self::PLATFORM_ROUTES) !== []) {
                $filtered[] = $item;
            }
        }
